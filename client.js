// DeepSeek Harness 动态插件「输入行为定制」— Client 半部
// 用法：在 DSH 会话中通过 cordis_define 定义插件时，将本文件内容作为 code.client。
// 职责：
//   - 接管系统输入快捷键（Enter / Ctrl+Enter / Shift+Enter / Alt+Enter / 发送按钮），
//     每个快捷键可独立配置行为：发送消息 / 繁忙时插入消息 / 换行 / 无作用。
//   - 繁忙时插入走系统 session.prompt(..., 'queue') 通道，与系统内置队列机制完全兼容。
//   - 发送失败时在输入栏正上方浮动显示错误提示。
//   - 设置页：设置 → 输入快捷键。
...
return {
  async apply(ctx) {
    ...
    try {
      const seeded = await host.call('composer-trigger-config-get')
      if (seeded && seeded.config) store.set(normalize(seeded.config))
      else host.call('composer-trigger-config-set', { config }).catch(() => {})
    } catch (err) {}
    const persist = (next) => {
      store.set(next)
      host.call('composer-trigger-config-set', { config: next }).catch(() => {})
    }
    const useConfig = () => {
    ...
    // ---------- 拦截组件：挂在输入框上方的 dock（提示浮动在输入栏正上方） ----------
