...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      // ----- durable settings (plugin-owned webServer route) -----
      // The api-gateway settings RPCs only expose the framework's hard-coded
      // namespace allowlist, so reads/writes travel over the plugin's own
      // /dsh-enter-customizer route instead. settingsScope is kept as a
      // best-effort fallback for UI synchronization only.
      let scope = undefined;
      ...
      const hostValue = { current: undefined, listeners: new Set() };
      const hostNotify = () => { for (const fn of Array.from(hostValue.listeners)) fn() };
      let hostReadGen = 0;
      const loadFromHost = () => {
        const gen = ++hostReadGen;
        fetch("/dsh-enter-customizer", { cache: "no-store" })
          .then((res) => res.json())
          .then((body) => {
            if (body && body.ok && gen === hostReadGen) {
              hostValue.current = body.value;
              hostNotify();
            }
          })
          .catch(() => {});
      };
      const writeToHost = (patch) => {
        hostReadGen += 1;
        fetch("/dsh-enter-customizer", {
          method: "POST",
      ...
      hostValue.listeners.add(() => {
        if (hostValue.current) store.set(normalize(hostValue.current));
      });
      loadFromHost();
      const persist = (next) => {
        ...
        const patch = {};
        for (const key of changed) patch[key] = next[key];
        writeToHost(patch);
        if (scope !== undefined) {
          for (const key of changed) scope.set(key, next[key]);
        }
      };
      const useConfig = () => {
